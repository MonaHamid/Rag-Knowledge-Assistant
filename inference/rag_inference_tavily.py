...
def context_is_relevant(question, candidates, threshold=RELEVANCE_THRESHOLD):
    """Compute real cosine similarity between question and retrieved chunks."""
    if not candidates:
        return False
...
def answer_with_rag_or_tavily(question):
    ...
    use_faiss = should_use_faiss(cands) and context_is_relevant(question, cands)

    if use_faiss:
        strong = sorted([c for c in cands if c["score"] >= FAISS_MIN_SCORE],
                        key=lambda x: x["score"], reverse=True)
        context = build_context(strong[:3])
        prompt = (
        ...
        weak_signals = ["don't know", "not found", "no information", "unavailable", "cannot answer"]
    ...
    # Fallback: FAISS irrelevant or too weak
    web = tavily_search(question)
    return "TAVILY", web
